...
export default function App() {
  ...
  useEffect(() => {
    ...
    const clockInterval = setInterval(() => {
      ...
      const optionsTime = { timeZone: "America/Sao_Paulo", hour: "2-digit", minute: "2-digit", second: "2-digit", hour12: false } as const;
      const optionsParts = { timeZone: "America/Sao_Paulo", hour: "2-digit", minute: "2-digit", hour12: false } as const;
      
      const timeString = liveServerTime.toLocaleTimeString("pt-BR", optionsTime);
      
      // Determine hour & minute for queue bounds check
      const formatterParts = new Intl.DateTimeFormat("pt-BR", optionsParts);
      const parts = formatterParts.formatToParts(liveServerTime);
      let hour = 0, minute = 0;
      parts.forEach(p => {
      ...
      // Get YYYY-MM-DD
      const yyyy = liveServerTime.getFullYear();
      const mm = String(liveServerTime.getMonth() + 1).padStart(2, "0");
      const dd = String(liveServerTime.getDate()).padStart(2, "0");
